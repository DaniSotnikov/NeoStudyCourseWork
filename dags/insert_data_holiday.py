...
def update_currency_code(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.loc[not_null_values, 'CURRENCY_CODE'] = (
        df.loc[not_null_values, 'CURRENCY_CODE']
        .astype(float)
        .astype(int)
        .astype(str)
...
def detect_encoding(file_path: str) -> str:
    ...
    try:
        with open(file_path, 'rb') as f:
            raw_data = f.read(
                1024 * 10
            )  # Читаем не весь фай, а только часть для определения кодировки
        result = detect(raw_data)
        return result['encoding']
    except Exception as e:
        logger.error(
            f'Не удалось определить кодировку для {file_path}: {traceback.format_exc()}'
        )
        raise
...
TABLE_PROCESSING_RULES = {
    'product_info': {
        'deduplication_keys': [],  # Столбцы подлежащие удалению дубликатов
        ...
        'type_casting': {  # Приведение типов
            'deal_start_date': {'type': 'date', 'format': '%Y-%m-%d'},
            'effective_from_date': {'type': 'date', 'format': '%Y-%m-%d'},
            'effective_to_date': {'type': 'date', 'format': '%Y-%m-%d'},
        },
    },
    'deal_info': {
        'deduplication_keys': [],
        'transformations': [],
        'validations': [],
        'type_casting': {'effective_from_date': {'type': 'date', 'format': '%Y-%m-%d'},
                         'effective_to_date': {'type': 'date', 'format': '%Y-%m-%d'}},
    },
}
...
def process_data(df: pd.DataFrame, table_name: str) -> pd.DataFrame:
    ...
    :return: Обработанный DataFrame
    """
    rules = TABLE_PROCESSING_RULES.get(
        table_name, {}
    )  # определяем набор правил для таблицы

    deduplication_keys = rules.get('deduplication_keys', [])
    if deduplication_keys:
        df = df.drop_duplicates(
            subset=deduplication_keys
        )  # Удаляем дубликаты
    ...
    if transformations:
        for (
                transform
        ) in transformations:  # Вызываем функции для трансформации
            df = transform(df)
        ...
        if column in df.columns:
            ...
            if cast_type == 'datetime':
                df[column] = pd.to_datetime(
                    df[column], format=cast_format, errors='coerce'
                )
            elif cast_type == 'date':
                df[column] = pd.to_datetime(
                    df[column], format=cast_format, errors='coerce'
...
def insert_data(table_name: str) -> None:
    ...
    log_id, start_time = log_process(
        table_name, 'In Progress', 'Loading started'
    )
    try:
        ...
        logger.info(
            f'Успешно проведена валидация, трансформация, приведение типов и удаление дубликатов'
        )
        df.columns = [col.lower() for col in df.columns]
        logger.info('Привели названия столбцов к нижнему регистру')
        deduplication_keys = TABLE_PROCESSING_RULES[table_name][
            'deduplication_keys'
        ]
        logger.info(
            f'Опеределены ключевые столбы для дедупликации: {deduplication_keys}'
        )
        postgres_hook = PostgresHook('postgres-db')
        ...
        logger.info(
            f'Определено название временной таблицы для UPSERT {temp_table_name}'
        )
        with engine.connect() as connection:
            ...
            """
            logger.info(
                f'Сгенерировали запрос для добавления записей {upsert_query}'
            )
            connection.execute(upsert_query)
            logger.info(f'UPSERT запрос завершен для {table_name}')

            connection.execute(
                f'DROP TABLE IF EXISTS ds.{temp_table_name};'
            )
        ...
        logger.error(
            f'Ошибка базы данных при загрузке данных для {table_name}: {e}'
        )
...
with DAG(
        ...
        description='Data loading to holidays',
